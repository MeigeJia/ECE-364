...
if (( $numInputs != 1 ))
then
	printf "Usage: sort_logs.bash <input file>\n"
	exit 1
fi
...
then
	if rm -f $1.unsorted
	then
		printf "Note: Removing existing file $1.unsorted.\n"
	else
		printf "Error: Could not remove $1.unsorted.\n"
	fi
fi
...
then
	if rm -f $1.sorted
	then
		printf "Note: Removing existing file $1.sorted.\n"
	else
		printf "Error: Could not remove $1.sorted.\n"
	fi
fi
...
do
	...
	then
		for (( i=0 ; i<$len ; i++ ))
		do
			echo "${nameList[$i]}, ${curData[0]}, ${tempData[$i]}" >> "$1.unsorted"
		done
	else
		nameList=("${data[@]:1}")
	fi
...
sort -t"," -k2,2 -o "$1.sorted" "$1.unsorted"
sort -t"," -k3,3r -o "$1.sorted" "$1.sorted"
